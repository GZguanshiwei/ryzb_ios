...
-(void)initData{
    ...
    [self.tableData addObject:@"支付密码设置"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        case 0:
        case 1:
            [self goChangePasswordVCWithIndex:indexPath.row];
            break;
        case 2:
            [self goBindPhoneVC];
            break;
    ...
    [tipView showViewWithDoneBlock:^(NSDictionary *params) {
        ...
        if(buttonIndex == 1){
            [JMCacheCleanHelper cleanCachesCompletion:^{
                [JMProgressHelper toastInWindowWithMessage:@"缓存清除成功"];
                NSString *cacahSize = [NSString stringWithFormat:@"清除缓存(%.2fM)",[JMCacheCleanHelper cachesSize]];

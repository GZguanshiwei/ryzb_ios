...
//我的直播间等级
@property (strong, nonatomic) LiveLevelModel *levelModel;
@property (strong, nonatomic) NSDate *zanDate;
@property (strong, nonatomic) ReceiveGiftManager *giftManager;

@property (assign, nonatomic) NSInteger sendMessageCount;// -1:表示已经完成任务
@end
...
-(void)initData{
    ...
    self.giftManager = [[ReceiveGiftManager alloc]init];
    self.giftManager.giftShowView = self.giftShowView;
...
- (void)stopRtmp {
    if (self.liveRoom) {
        [self.liveRoom exitRoom:self.roomDetail.roomId isCreator:NO completion:nil];
    }
...
- (void)onRecvRoomCustomMsg:(NSString *)roomID userID:(NSString *)userID userName:(NSString *)userName userAvatar:(NSString *)userAvatar cmd:(NSString *)cmd message:(NSString *)message level:(NSString *)level{
    TCMsgModel *msg = [[TCMsgModel alloc] init];
    msg.userId = userID;
    msg.userHeadImageUrl = userAvatar;
    ...
    switch (msg.msgType) {
        case TCMsgModelType_MemberEnterRoom:
            msg.userMsg = @"加入房间";
            if (level.intValue == 4 || level.intValue == 5) {
            [self.giftManager setupGiftAnim:msg];
             }
            break;
    ...
    if (msg.msgType != TCMsgModelType_Gift) {
        [self handleIMMessage:msg];
    }
}
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    msgModel.liveGrade = self.levelModel.grade;
    [[MLVBLiveRoom sharedInstance] sendRoomTextMsg:msgModel.userMsg completion:^(int errCode, NSString *errMsg) {
        ...
        if (errCode == 10017) {
            ...
            if(self.sendMessageCount != -1){
                self.sendMessageCount++;
                if(self.sendMessageCount > 10){
                    [self requestGetIronValue];
                }
            }
            [self handleIMMessage:msgModel];
        }
...
- (IBAction)giftAction:(id)sender {
    if ([[JMProjectManager sharedJMProjectManager] isTourist] == NO){
        ...
        giftVC.successBlock = ^(NSDictionary * _Nonnull params) {
            [self.liveRoom sendRoomCustomMsg:[@(TCMsgModelType_Gift) stringValue] msg:[params mj_JSONString] completion:nil];
            
            JMLoginUserModel *loginUser = [JMProjectManager sharedJMProjectManager].loginUser;
            TCMsgModel *msg = [[TCMsgModel alloc] init];
            msg.userId = loginUser.userId;
            msg.userHeadImageUrl = loginUser.headUrl;
            NSString *name = loginUser.nickName;
            msg.userName = name.length > 0 ? name : loginUser.userId;
            msg.msgType = TCMsgModelType_Gift;
            msg.liveGrade = self.levelModel.grade;
            msg.userMsg = [params mj_JSONString];
            [self.giftManager setupGiftAnim:msg];
...
-(void)requestRoomInfo{
    ...
    [[JMRequestManager sharedManager] POST:kUrlRoomInfo parameters:params completion:^(JMBaseResponse *response) {
        ...
        if(codeStr.intValue == 0){
            ...
            [[JMProjectManager sharedJMProjectManager] closeEnterError:self.roomId];
        }else{
            NSDictionary *dataDic = response.responseObject[@"data"];
            NSArray *historyMessage = dataDic[@"msgs"];
            [self showChatHistory:historyMessage];
...
-(void)showChatHistory:(NSArray<NSDictionary *> *)array{
    for(NSDictionary *dic in array){
        TCMsgModel *msg = [[TCMsgModel alloc] init];
//        msg.userId = userID;
        msg.userHeadImageUrl = [dic getJsonValue:@"headPic"];
        msg.userName = [dic getJsonValue:@"nickName"];
        msg.msgType = TCMsgModelType_NormalMsg;
        msg.userMsg = [dic getJsonValue:@"text"];
        msg.liveGrade = [dic getJsonValue:@"level"];
        if(self.messageArray == nil){
            self.messageArray = [[NSMutableArray alloc] init];
        }
        [self.messageArray addObject:msg];
    ...
    for(int i=0;i<self.messageArray.count;i++){
        NSIndexPath* insertion = [NSIndexPath indexPathForRow:i inSection:0];
        [CATransaction begin];
        ...
        [self.messageTableView insertRowsAtIndexPaths:@[insertion] withRowAnimation:UITableViewRowAnimationNone];
...
-(void)requestGetIronValue{
    //直播间发言10条获取铁值
    NSMutableDictionary *params = [JMCommonMethod baseRequestParams];
    [params setJsonValue:self.roomId key:@"roomId"];
    [params setJsonValue:@"6" key:@"type"];

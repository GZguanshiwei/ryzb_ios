...
- (CAEmitterLayer *)emitterLayer
{
    if (!_emitterLayer) {
        ...
        CGFloat cellWidth = (kScreenWidth - 36)/2.0;
        CGFloat cellHeight = 270;
        
        emitterLayer.emitterPosition = CGPointMake(cellWidth-30, cellHeight-40);
        // 发射器的尺寸大小
        emitterLayer.emitterSize = self.zanButton.size;
        // 渲染模式
        ...
        for (int i = 0; i<5; i++) {
            ...
            // 粒子存活时间
            stepCell.lifetime = arc4random_uniform(5)  ;
            // 粒子的生存时间容差
            stepCell.lifetimeRange = 1.5;
            // 颜色
            // fire.color=[[UIColor colorWithRed:0.8 green:0.4 blue:0.2 alpha:0.1]CGColor];
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"zanSmall_0%d", i+1]];
            // 粒子显示的内容
            stepCell.contents = (id)[image CGImage];
            // 粒子的名字
            //            [fire setName:@"step%d", i];
            // 粒子的运动速度
            stepCell.velocity = arc4random_uniform(150);
            // 粒子速度的容差

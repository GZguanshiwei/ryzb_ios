...
-(void)initControl{
    ...
    if (self.type == UCChangePasswordViewControllerTypePay) {
        ...
        if (loginUser.hasPayPassword == NO) {
...
- (IBAction)resetAction:(id)sender {
    ...
    switch (self.type) {
        case UCChangePasswordViewControllerTypePay:
        {
            if(password.length != 6){
                [JMProgressHelper toastInWindowWithMessage:@"请输入长度为6位的支付密码"];
                return;
            }
            //修改支付密码
            ...
            break;
        case UCChangePasswordViewControllerTypeLogin:
            //修改登录密码
...
-(void)requestUpdatePayPassword{
    ...
    [[JMRequestManager sharedManager] POST:kUrlUpdatePayPwd parameters:params completion:^(JMBaseResponse *response) {
        if(response.error){
            ...
            JMLoginUserModel *loginUser = [JMProjectManager sharedJMProjectManager].loginUser;
            NSString *tip = loginUser.hasPayPassword?@"修改密码成功":@"设置密码成功";
            [[JMProjectManager sharedJMProjectManager] updateLoginUserInfoSuccessBlock:^{
                [JMProgressHelper toastInWindowWithMessage:tip];
                [self.navigationController popViewControllerAnimated:YES];
            } failBlock:nil];
        }
    }];

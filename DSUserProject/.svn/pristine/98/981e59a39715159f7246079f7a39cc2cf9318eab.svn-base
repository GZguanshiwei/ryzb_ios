...
-(void)setShopOrderData:(OrderModel *)shopOrderData{
    _shopOrderData = shopOrderData;
    [self.coverImageView sd_setImageWithURL:[JMCommonMethod imageUrlWithPath:self.shopOrderData.good.coverImage]];
    self.titleLabel.text = self.shopOrderData.good.title;
    self.priceLabel.text = [NSString stringWithFormat:@"￥%@",self.shopOrderData.good.price];
    self.totalPayLabel.text = [NSString stringWithFormat:@"￥%@",self.shopOrderData.totalPay];
    self.orderNoLabel.text = [NSString stringWithFormat:@"订单号：%@",self.shopOrderData.orderNo];
    if(self.shopOrderData.afterSaleState == JMOrderAfterSale_InAudit){
        self.tagView.hidden = NO;
    }else{
        self.tagView.hidden = YES;
    }
    [self shopOrderUpdateStateTipAndOperationButton];
}

-(void)shopOrderUpdateStateTipAndOperationButton{
    NSString *stateTip = @"";
    NSMutableArray *buttonTitles = [[NSMutableArray alloc] init];
    switch (self.shopOrderData.state) {
        case JMOrderState_Close:
            stateTip = @"交易关闭";
            break;
        case JMOrderState_WaitPay:

...
-(void)payOrder:(NSInteger)index{
    OrderModel *order = self.tableData[index];
    if(order.orderType.integerValue == 1){
        [self goDetailVC:order.orderId];
    }else{
        [self goPayVC:order];
    }
...
-(void)goPayVC:(OrderModel *)order{
    if(order.payType.intValue == 5){
        //分次支付
        if(JMIsEmpty(order.manyTimesPayNo)){
            ...
            [[JMRequestManager sharedManager] POST:kUrlPayManyTimeCreate parameters:params completion:^(JMBaseResponse *response) {
                if(response.error){
                    ...
                    order.manyTimesPayNo = [dataDic getJsonValue:@"orderNo"];
                    [self goManyTimesPay:order];
                }
            }];
        }else{
            [self goManyTimesPay:order];

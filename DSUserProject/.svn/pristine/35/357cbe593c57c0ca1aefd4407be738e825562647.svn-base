...
/**
 ...
 @param level 等级
 */
-(void)setLiveLevel:(NSString *)level{
    [_msgMgr setLiveLevel:level];
}
...
/**
 ...
 2. 播放房间的混流播放地址
 */
- (void)enterRoom:(NSString *)roomID playUrl:(NSString *)playUrl view:(UIView *)view completion:(void(^)(int errCode, NSString *errMsg))completion {
    __weak __typeof(self) weakSelf = self;
    ...
    [self asyncRun:^(MLVBLiveRoom *self){
        ...
        self.roomInfo.mixedPlayURL = playUrl;
        self.roomInfo.roomCreator = roomID;
...
-(void)exitRoom:(NSString *)roomId isCreator:(BOOL)isCreater completion:(void(^)(int errCode, NSString *errMsg))completion{
    __weak __typeof(self) weakSelf = self;
    [self asyncRun:^(MLVBLiveRoom *self){
...
-(void)onRecvGroupTextMsg:(NSString *)groupID userID:(NSString *)userID textMsg:(NSString *)textMsg userName:(NSString *)userName userAvatar:(NSString *)userAvatar level:(NSString *)level{
    if (![groupID isEqualToString:self.roomInfo.roomID]) {
        return;
    }
    
    dispatch_async(self.delegateQueue, ^{
        if([self.delegate respondsToSelector:@selector(onRecvRoomTextMsg:userID:userName:userAvatar:message:level:)]){
            [self.delegate onRecvRoomTextMsg:groupID userID:userID userName:userName userAvatar:userAvatar message:textMsg level:level];
        }
    });
}
...
// 接收群自定义消息，cmd为自定义命令字，msg为自定义消息体(这里统一使用json字符串)
- (void)onRecvGroupCustomMsg:(NSString *)groupID userID:(NSString *)userID cmd:(NSString *)cmd msg:(NSString *)msg userName:(NSString *)userName userAvatar:(NSString *)userAvatar level:(NSString *)level{
    if (![groupID isEqualToString:self.roomInfo.roomID]) {
        return;
    }
    
    dispatch_async(self.delegateQueue, ^{
        if ([self.delegate respondsToSelector:@selector(onRecvRoomCustomMsg:userID:userName:userAvatar:cmd:message:level:)]) {
            [self.delegate onRecvRoomCustomMsg:groupID userID:userID userName:userName userAvatar:userAvatar cmd:cmd message:msg level:level];
        }
    });
}

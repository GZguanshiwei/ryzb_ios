...
- (void)timeCount{
    //已发货，仅退款:7天   未发货，仅退款:3天
    NSDate *startTime;
    NSDate *endTime;
    switch (self.order.type.integerValue) {
        case 0:
            startTime = [NSDate dateWithString:self.order.applyTime format:@"yyyy-MM-dd HH:mm:ss"];
            endTime = [startTime dateByAddingDays:3];
            break;
        case 1:
        case 2:
            startTime = [NSDate dateWithString:self.order.applyTime format:@"yyyy-MM-dd HH:mm:ss"];
            endTime = [startTime dateByAddingDays:7];
            break;
        default:
            break;
    }
    ...
    NSString *showTime = [NSString stringWithFormat:@"%ld天%ld小时后系统自动为您退款",days,hours];
    NSMutableAttributedString *attri = [[NSMutableAttributedString alloc] initWithString:showTime];
    [attri addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"#ED1731"] range:NSMakeRange(0, showTime.length-9)];

...
//        NSDictionary *customMsg = @{@"userName":self->_config.userName, @"userAvatar":self->_config.userAvatar, @"cmd": CMD_CUSTOM_CMD_MSG, @"data": data};
        ...
        NSDictionary *data = @{@"cmd": cmd, @"msg": msg == nil ? @"" : msg, @"userName": self->_config.userName, @"userAvatar": self->_config.userAvatar, @"level":self->_config.liveLevel};
        NSDictionary *customMsg = @{@"userName":self->_config.userName, @"userAvatar":self->_config.userAvatar, @"cmd": CMD_CUSTOM_CMD_MSG, @"data": data};
...
- (void)sendGroupTextMsg:(NSString *)textMsg  completion:(void (^)(int errCode, NSString *errMsg))completion {
    [self asyncRun:^{
        ...
        NSDictionary *userInfo = @{@"nickName": self->_config.userName, @"headPic": self->_config.userAvatar, @"level":self->_config.liveLevel};
    ...
    for (int idx = 0; idx < [msg elemCount]; ++idx) {
        ...
        if ([elem isKindOfClass:[TIMCustomElem class]]) {
            ...
            if (cmd && [cmd isEqualToString:CMD_CUSTOM_CMD_MSG] && [data isKindOfClass:[NSDictionary class]]) {
                if (_delegate && [_delegate respondsToSelector:@selector(onRecvGroupCustomMsg:userID:cmd:msg:userName:userAvatar:level:)]) {
                    [_delegate onRecvGroupCustomMsg:_groupID userID:msg.sender cmd:data[@"cmd"] msg:data[@"msg"] userName:data[@"userName"] userAvatar:data[@"userAvatar"] level:data[@"level"]];
                }
            ...
            if ([cmd isEqualToString:CMD_CUSTOM_TEXT_MSG] && [data isKindOfClass:[NSDictionary class]]) {
                ...
                if (_delegate) {
                    [_delegate onRecvGroupTextMsg:_groupID userID:msg.sender textMsg:msgText userName:nickName userAvatar:headPic level:level];
                }
            } else if ([cmd isEqualToString:CMD_PUSHER_CHANGE]) {
                [_delegate onMemberChange:_groupID];
            }
...
//    }
    if ([[JMProjectManager sharedJMProjectManager].loginUser.sessionId isEqualToString:@"sessionId"]) {
        return;
    }
    ...
    [JXTAlertController jm_showAlertWithTitle:@"提示" message:@"您的帐号在别的设备上登录，您被迫下线！" appearanceProcess:^(JXTAlertController * _Nonnull alertMaker) {

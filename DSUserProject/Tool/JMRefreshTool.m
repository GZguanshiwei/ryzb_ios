...
//    MJRefreshAutoNormalFooter
    ...
    // 设置颜色
    footer.stateLabel.textColor = kGetColor(153, 153, 153);
    return footer;
}
...
-(MJRefreshNormalHeader *)refreshHeader{
    ...
    header.stateLabel.textColor = kGetColor(153, 153, 153);
...
-(void)requestData{
    ...
    if(self.isRefreshFooter == YES){
        ...
        [params setJsonValue:[NSString stringWithFormat:@"%ld",kGlobalPageSize] key:@"pageSize"];
    }
    ...
    [[JMRequestManager sharedManager] POST:self.requestUrl parameters:params completion:^(JMBaseResponse *response) {
        ...
        if(response.error){
            ...
            if(self.isRefreshFooter == YES){
                if(array.count < kGlobalPageSize){
